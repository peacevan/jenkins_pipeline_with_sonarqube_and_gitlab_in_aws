...
```properties
# Configurações do banco de dados PostgreSQL (use variáveis/segredos, não valores fixos)
sonar.jdbc.username=<SONAR_DB_USER>
sonar.jdbc.password=<SONAR_DB_PASSWORD>
sonar.jdbc.url=jdbc:postgresql://localhost:<POSTGRES_PORT>/<SONAR_DB_NAME>
```
...
- **Erro: Cannot assign requested address**
  ...
  Usuário e senha do PostgreSQL (exemplo — não use valores reais no repositório):
  - Usuário: <SONAR_DB_USER>
  - Senha: <SONAR_DB_PASSWORD>
  - Porta: <POSTGRES_PORT>
...
```sh
# Defina a URL externa do GitLab antes da instalação (ex.: https://gitlab.example.com)
sudo EXTERNAL_URL="<GITLAB_EXTERNAL_URL>" apt install gitlab-ce
```
